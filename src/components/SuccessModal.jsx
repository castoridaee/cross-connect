  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[2000] p-4 sm:p-6">
      ...
      >
        ...
        {/* Tab Switcher */}
        <div className={`flex-shrink-0 flex justify-center gap-2 mb-4 bg-slate-50 p-1 rounded-2xl w-fit mx-auto border border-slate-100/50 h-fit transition-all duration-500 z-30 ${activeTab === 'results' ? 'mt-4' : 'mt-2'}`}>
          <button
            ...
            <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-4 duration-500 fill-mode-both overflow-hidden">
              ...
              <div className="flex justify-between items-center mb-4 px-1 flex-shrink-0">
                ...
                <div className="relative">
                  ...
                  >
                    <Filter size={10} /> {commentSort === 'newest' ? 'Newest' : 'Top Liked'}
                    <ChevronDown size={10} className={`transition-transform ${isSortOpen ? 'rotate-180' : ''}`} />
                  </button>
                    ...
                    <>
                      ...
                      <div className="absolute right-0 top-full mt-1 w-32 bg-white border border-slate-100 rounded-xl shadow-xl z-50 p-1 overflow-hidden border-t-2 border-t-slate-900 animate-in fade-in zoom-in duration-200">
                        <button 
                          ...
                          className={`w-full text-left px-3 py-2 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-colors ${commentSort === 'newest' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50'}`}
                        >
                          Newest First
                        </button>
                        <button 
                          onClick={() => { setCommentSort('liked'); setIsSortOpen(false); }}
                          className={`w-full text-left px-3 py-2 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-colors ${commentSort === 'liked' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50'}`}
                        >
                          Most Liked
                        </button>
                      </div>
              ...
              {loadingComments && comments.length === 0 ? (
                <div className="flex-grow flex items-center justify-center">
                  <div className="animate-pulse font-black text-slate-200 text-xs">LOADING...</div>
                </div>
              ) : comments.length === 0 ? (
                <div className="flex-grow flex flex-col items-center justify-center py-8 text-center bg-white rounded-2xl border border-slate-100 shadow-sm px-4 overflow-hidden">
                  ...
                  <p className="text-xs font-bold text-slate-300 max-w-[240px]">Be the first to share your thoughts!</p>
                </div>
              ) : (
                <div 
                  ref={scrollRef}
                  ...
                  >
                    <div className="relative flex-grow">
                      <textarea 
                        ...
                        className="w-full bg-white border border-slate-100 rounded-2xl p-4 text-[11px] font-bold text-slate-700 resize-none outline-none focus:border-indigo-600 transition-all shadow-inner h-[52px]"
                      />
                    </div>
                    <button 
                      type="submit"
                      disabled={!newComment.trim() || isSubmitting}
                      className={`px-4 rounded-2xl text-white transition-all active:scale-95 flex items-center justify-center h-[52px] ${
                        newComment.trim() && !isSubmitting ? 'bg-indigo-600 shadow-lg shadow-indigo-100' : 'bg-slate-200 cursor-not-allowed'
                      }`}
                    >
                      <Send size={18} />
                    </button>
        ...
        <div className={`flex-shrink-0 grid transition-all duration-500 ease-in-out ${activeTab === 'results' ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0 pointer-events-none'} ${activeTab === 'comments' && !isAnimating ? 'hidden' : ''}`}>
          <div className="overflow-hidden">
            <div className={`space-y-2.5 sm:space-y-3 pb-1 transition-all duration-500 ${activeTab === 'results' ? 'mt-0' : 'mt-4'}`}>
